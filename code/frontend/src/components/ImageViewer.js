import React from "react";
import {Tabs, Image, Divider} from 'antd';
import ThreeDModelPresenter from '../components/3DModel/ThreeDModelPresenter';
import modelIcon from '../assets/3dmodel.png';
import {
    RotateLeftOutlined,
    ZoomInOutlined,
    DragOutlined,
} from '@ant-design/icons';

const {TabPane} = Tabs;
const fallbackSrc = "data:image/png;base64,";

const ImageViewer = ({imageList, height, width}) => {
    if (imageList == null || imageList.length === 0) {
        return (
            <div style={{height: height, width: width}}>
                <div style={{height: height, width: width}}>
                    <Tabs tabPosition="left" style={{height: height}}>
                        <TabPane key={1} tab={<Image data-testid="buttonImage" width="50" height="50"
                                                     style={{borderRadius: "10%"}} src={fallbackSrc}/>}>
                            <Image data-testid="displayImage" width={"100%"} style={{borderRadius: "5%"}}
                                   src={fallbackSrc}/>
                        </TabPane>
                    </Tabs>
                </div>
            </div>
        );
    } else {
        try {
            // Creates The 3D Model Tabs
            const tab3DList = imageList.filter((imageInfo) => imageInfo.is3DModelType === "Y").map((imageInfo) => {
                    return imageInfo.is3DModelType === "Y"
                        ?
                        <TabPane key={"3D" + imageInfo.id.toString()}
                                 tab={<Image preview={false} width="50" height="50" style={{borderRadius: "10%"}}
                                             src={modelIcon} alt="3D Model"/>}>
                            <div style={{height: height * 0.75, width: '100%'}}>
                                <div>
                                    <ThreeDModelPresenter modelUrl={imageInfo.threeDModelLocation}/>
                                </div>
                                <div>
                                    <Divider>Instructions</Divider>
                                    <RotateLeftOutlined/> Click and Drag to Rotate <br/>
                                    <ZoomInOutlined/> Scroll to Zoom <br/>
                                    <DragOutlined/> Arrow Keys to Pan
                                </div>
                            </div>
                        </TabPane>
                        :
                        <></>
                }
            );

            // Creates the Image Tabs
            const tabImageList = imageList.filter((imageInfo) => imageInfo.is3DModelType === "N").map((imageInfo) => {
                    return imageInfo.is3DModelType === "N"
                        ?
                        <TabPane key={imageInfo.id.toString()}
                                 tab={<Image preview={false} width="50" height="50" style={{borderRadius: "10%"}}
                                             src={imageInfo.smallImageLocation} fallback={fallbackSrc}/>}>
                            <Image
                                width={"100%"}
                                style={{borderRadius: "5%"}}
                                src={imageInfo.largeImageLocation}
                                fallback={fallbackSrc}
                            />
                        </TabPane>
                        :
                        <></>
                }
            );


            // Returns the Tabs for the image viewer
            return (
                <div style={{height: height, width: width}}>
                    <Tabs tabPosition="left" style={{height: height}}>
                        {tab3DList}
                        {tabImageList}
                    </Tabs>
                </div>
            );
        } catch (error) {
            return (

                <div style={{height: height, width: width}}>
                    <div style={{height: height, width: width}}>
                        <Tabs tabPosition="left" style={{height: height}}>
                            <TabPane key={1} tab={<Image width="50" height="50" style={{borderRadius: "10%"}}
                                                         src={fallbackSrc}/>}>
                                <Image width={"100%"} style={{borderRadius: "5%"}} src={fallbackSrc}/>
                            </TabPane>
                        </Tabs>
                    </div>
                </div>

            );
        }
    }
}


export default ImageViewer
